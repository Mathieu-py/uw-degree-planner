-- Replace free-form grades with a coarse credit outcome.
--
-- plan_courses.grade stored raw transcript tokens — including actual
-- percentages — while the app only ever consumes a pass/fail-shaped signal
-- (audit placement, taken-vs-planned display). Storing real grades is a
-- privacy liability with no benefit, so the column becomes `outcome`:
-- 'credit' | 'noCredit' | 'inProgress' | 'transfer', null = planned. Literals
-- mirror OUTCOME_VALUES (lib/plan/types.ts) verbatim — no mapping layer.
--
-- Snapshot course shape after this migration: { code: text, outcome?: text }.

alter table public.plan_courses rename column grade to outcome;

-- Null passes an IN check, so planned courses are unaffected. The constraint
-- guards direct RLS-permitted table writes; the RPC below can't violate it.
alter table public.plan_courses
  add constraint plan_courses_outcome_valid
  check (outcome in ('credit', 'noCredit', 'inProgress', 'transfer'));

-- ---------------------------------------------------------------------------
-- Recreate save_plan_state for the outcome column. Body is otherwise identical
-- to migrations/0006 — only the plan_courses insert changes. (Functions can't
-- be patched in place; a full `create or replace` is the migration idiom.)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Recreate get_shared_plan for the outcome column. Body otherwise identical to
-- migrations/0006 — only the course key changes.
-- ---------------------------------------------------------------------------
