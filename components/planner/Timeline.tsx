...
export function Timeline({
  ...
  return (
    <div className="flex flex-col gap-3">
      {preSlot && preSlot.courses.length > 0 ? (
        <div className="rounded-lg border border-zinc-200 dark:border-zinc-800 bg-amber-50/60 dark:bg-amber-950/30 px-4 py-3">
          <div className="text-xs uppercase tracking-wider text-amber-700 dark:text-amber-300 mb-1">
            Transfer credits
          </div>
          <div className="flex flex-wrap gap-1.5">
            {preSlot.courses.map((c) => (
              <span
                key={c.code}
